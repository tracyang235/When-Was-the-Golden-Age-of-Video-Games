...
get_ipython().run_cell_magic('sql', '', '\n-- Join games_sales and reviews\n-- Select a count of the number of games where both critic_score and user_score are null\nSELECT count(*)\nFROM game_sales s LEFT JOIN reviews on s.game = reviews.game\nWHERE critic_score IS NULL AND user_score IS NULL')
...
get_ipython().run_cell_magic('sql', '', '\n-- Select the year results that appear on both tables\nSELECT u.year\nFROM top_user_years_more_than_four_games u JOIN top_critic_years_more_than_four_games c\nON u.year = c.year')
...
get_ipython().run_cell_magic('sql', '', '\n-- Select year and sum of games_sold, aliased as total_games_sold; order results by total_games_sold descending\n-- Filter game_sales based on whether each year is in the list returned in the previous task\nSELECT year, SUM(g.games_sold) AS total_games_sold\nFROM game_sales g\nGROUP BY g.year\nHAVING g.year = 1998 OR g.year = 2008 OR g.year = 2002\nORDER BY total_games_sold DESC')
